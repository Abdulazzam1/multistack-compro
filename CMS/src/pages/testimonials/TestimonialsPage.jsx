...
export default function TestimonialsPage() {
  ...
  const openEdit = (t) => {
    setForm({ client_name:t.client_name||'', client_title:t.client_title||'',
              client_company:t.client_company||'', content:t.content||'',
              rating:t.rating||5, is_active:t.is_active!==false });
    setEditId(t.id); setOpen(true);
  };
  const closeModal = () => { setOpen(false); setEditId(null); };

  const handleSave = async () => {
    if (!form.client_name.trim() || !form.content.trim()) return alert('Nama klien dan isi testimoni wajib diisi.');
    setSaving(true);
    try {
              ...
              <div key={t.id} className={`card ${styles.card}`}>
                <div className={styles.cardTop}>
                  ...
                  <div className={styles.cardActions}>
                    <button onClick={() => openEdit(t)} className="btn btn-ghost btn-sm"><Pencil size={13}/></button>
                    <button onClick={() => handleDelete(t)} className="btn btn-danger btn-sm"><Trash2 size={13}/></button>
                  </div>
                </div>
                <p className={styles.content}>"{t.content}"</p>
                <div className={styles.author}>
                  <div className={styles.avatar}>{(t.client_name||'?').slice(0,2).toUpperCase()}</div>
                  <div>
