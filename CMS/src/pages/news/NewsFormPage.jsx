...
export default function NewsFormPage() {
  ...
  // ── Fetch by ID via endpoint khusus admin ────────────────────────────────
  // Endpoint: GET /api/news/id/:id  (butuh token, tanpa filter is_published)
  // Berbeda dengan GET /api/news/:slug yang hanya untuk FE publik
  const { data: existing, isLoading: loadingExisting } = useQuery({
    queryKey:        ['news-edit', id],
    queryFn:         () => api.get(`/news/id/${id}`).then(r => r.data.data),
    enabled:         isEdit,
    staleTime:       0,
    refetchOnMount:  true,
  });

  // Isi semua field form dari data existing — tanpa initialized.current
  // agar form selalu sinkron dengan data terbaru dari server
  useEffect(() => {
    if (!existing) return;
    setForm({
      ...
      author:       existing.author       ?? 'Admin Multistack',
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      alert('Upload gagal: ' + getErrorMsg(err));
    } finally {
      setUploading(false);
    }
  };
  ...
  // Tampilkan loading saat data edit belum tiba
  if (isEdit && loadingExisting) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '60vh', color: 'var(--white70)' }}>
        Memuat data berita...
      </div>
    );
  }
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div className={`card ${styles.section}`}>
            ...
            <label className={`btn btn-outline ${styles.uploadBtn}`}>
              ...
              {uploading
                ? 'Mengupload...'
                : form.cover_image ? 'Ganti Foto Cover' : 'Upload Foto Cover'}
              <input
                type="file"
                accept="image/*"
