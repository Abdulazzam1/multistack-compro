...
export default function ServicesPage() {
  ...
  const openNew  = () => { setForm(EMPTY); setEditData(null); setOpen(true); };
  const openEdit = (s) => {
    setForm({ name:s.name||'', description:s.description||'', scope:s.scope||'',
              icon:s.icon||'', image:s.image||'', sort_order:s.sort_order||0, is_active:s.is_active!==false });
    setEditData(s);
    setOpen(true);
  };
            ...
            <div className={modal.body}>
              <div className={modal.row2}>
                <div>
                  ...
                  <input className="form-input" value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))} placeholder="HVAC & Refrigerasi" />
                </div>
                <div>
              ...
              <div>
                ...
                {form.image && (
                  <div className={modal.imgPreview}>
                    <img src={imgUrl(form.image)} alt="preview" />
                  </div>
                )}
                <label className={`btn btn-outline ${modal.uploadBtn}`}>
                  <Upload size={14}/>{uploading ? 'Mengupload...' : form.image ? 'Ganti Foto' : 'Upload Foto'}
            ...
            : (
              <table style={{width:'100%'}}>
                <thead style={{background:'var(--bg3)',borderBottom:'1px solid var(--border)'}}>
                  <tr>
                    <th className="th" style={{width:60}}>No</th>
                ...
                <tbody>
                  {services.length === 0
                    ? <tr><td className="td" colSpan={5} style={{textAlign:'center',padding:'3rem',color:'var(--white70)'}}>Belum ada layanan. <button className="btn btn-outline btn-sm" onClick={openNew} style={{marginLeft:'0.5rem'}}>Tambah sekarang</button></td></tr>
                    : services.map(s => (
                        <tr key={s.id}>
                          <td className="td" style={{fontFamily:'var(--font-mono)',fontSize:'0.78rem',color:'var(--white40)'}}>{s.sort_order}</td>
                          ...
                          <td className="td">
                            ...
                            {s.description && <p style={{fontSize:'0.73rem',color:'var(--white40)',overflow:'hidden',whiteSpace:'nowrap',textOverflow:'ellipsis',maxWidth:360}}>{s.description}</p>}
                          </td>
                          <td className="td">
